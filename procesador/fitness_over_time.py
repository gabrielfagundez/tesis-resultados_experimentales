  ...
  for tamano in ['chicas', 'medianas', 'grandes', 'montevideo']:
    for ins in ['1', '2', '3', '4', '5', '6']:
      if not(tamano == 'montevideo' and ins == '5' or tamano == 'montevideo' and ins == '6'):
        ...
        # Dibujo la grafica
        plt1, = plt.plot(data[tamano][ins]['clei']['tiempo']['1'], data[tamano][ins]['clei']['fitness']['1'])
        plt2, = plt.plot(data[tamano][ins]['alio']['tiempo']['1'], data[tamano][ins]['alio']['fitness']['1'])

        plt.ylabel('Fitness')
        plt.xlabel('Tiempo(s)')
        plt.legend([plt1, plt2], ['Algoritmo Secuencial', 'Algoritmo Paralelo'])
